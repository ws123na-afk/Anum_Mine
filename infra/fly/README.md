...
2. **Decide the login flow** (see section 6 — this is the one item on this
   list that is a real design decision, not a mechanical edit). The
   template keeps `anum-web` as a public client with
   `directAccessGrantsEnabled: false`, i.e. no login path is currently
   wired up for it at all. Either:
   - Build a real Authorization Code + PKCE flow into `apps/web` (not done
     anywhere in this repo yet) and leave the client as a public client
     with `standardFlowEnabled: true`, or
   - If tokens will instead be minted by some backend/service process
     rather than an end-user's browser, switch `anum-web` (or add a new
     client) to a confidential client: `"publicClient": false`, a real
     client secret generated by Keycloak (not put in this JSON — see
     below), `"standardFlowEnabled": true` or `"serviceAccountsEnabled":
...
deterministic result. Two things in this runbook are deliberately *not*
automated, because no amount of tooling substitutes for a real decision
here:

- **How real end users actually log in.** The only working login path
  anywhere in this repo today is the *local dev realm's* Resource Owner
  Password Credentials grant (`directAccessGrantsEnabled: true` on the dev
  realm's `anum-web` client, used by `infra/docker/keycloak/README.md`'s
  `curl` example to fetch a test token) — a convenience for hitting the API
  with `curl` during development, not something a real browser-based end
  user should ever do (it means the login form, wherever it lives, has to
  see and handle the user's raw password, defeating most of the point of
  using an external identity provider). The production template
  deliberately disables it (`directAccessGrantsEnabled: false`) and ships
  with **no login flow wired up at all** — no Authorization Code + PKCE
  redirect exists anywhere in `apps/web`. Building that (or deciding tokens
  will instead come from a confidential-client backend flow — see section
  3, step 2) is real frontend/auth design work, not a config edit, and
  nothing in this directory does it for you.

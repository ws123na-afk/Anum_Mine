...
- **Skills and tools, wired into the live run loop**: a skill-manifest registry (`anum_api/skills.py`, two shipped skills) is consulted at the start of every task run — a match is recorded as a run step — and the high-risk-action path in `anum_api/runtime.py` goes through the real tool contract + mediation framework (`anum_api/tools.py`: risk-gated approval, audit trail) instead of hand-rolling an `Approval`. Internal tools, one REST integration adapter, and one MCP-style adapter path are also built and tested (`internal_tools.py`, `rest_integration.py`, `mcp_adapter.py`), ready for a real deployment to register more of them.
- **A real model provider adapter** (`ANUM_MODEL_PROVIDER=anthropic` + `ANUM_ANTHROPIC_API_KEY`): `anum_api/model_gateway.py`'s `AnthropicModelGateway` implements real text generation, structured output (forced tool-choice), and streaming against the Anthropic API — unset (the default, `mock`), every task run still uses the deterministic Phase 1 `MockModelGateway`, unchanged.
